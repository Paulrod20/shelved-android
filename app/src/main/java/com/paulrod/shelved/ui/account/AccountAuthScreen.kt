        ...
        Column(
            ...
            if (state.verificationEmail != null) {
                ...
                AccountAuthForm(
                    ...
                    onCreateAccount = { onCreateAccount(email, password, confirmation) },
                    onEmailSignIn = { onEmailSignIn(email, password) },
                    onPasswordReset = { onPasswordReset(email) },
                )
            }
        }
    }
...
private fun AccountAuthForm(
    ...
    password: String,
    confirmation: String,
    onModeChanged: (EmailAuthMode) -> Unit,
    ...
    val isCreatingAccount = mode == EmailAuthMode.CREATE_ACCOUNT
    PageTitle(
        stringResource(
            if (isCreatingAccount) R.string.account_create_title else R.string.account_sign_in_title,
        ),
    )
    PageBody(
        stringResource(
            if (isCreatingAccount) R.string.account_create_body else R.string.account_sign_in_body,
        ),
    )
    ...
    AuthModeSelector(mode, onModeChanged)
    ...
    AuthPasswordField(
        ...
        keyboardActions = KeyboardActions(onDone = { if (!isCreatingAccount) onEmailSignIn() }),
    )
    if (isCreatingAccount) {
        ...
        AuthNote(stringResource(R.string.onboarding_password_note))
    } else {
        AuthTextButton(
    ...
    AuthPrimaryButton(
        label = stringResource(
            if (isCreatingAccount) R.string.onboarding_create_account else R.string.onboarding_sign_in,
        ),
        enabled = !state.isLoading,
        loading = state.isLoading,
        onClick = if (isCreatingAccount) onCreateAccount else onEmailSignIn,
    )
}
...
@Composable
private fun AuthNote(text: String) {
    Text(
        text,
        color = TextMuted,
        fontSize = 11.sp,
